...
/// Generate a test [`NoteHeader`] with a random sender
...
/// Generate a mock P2ID [`Note`] with a random sender
...
/// Create a mock P2ID note with specified tag, sender and target account IDs
...
/// Replace given a `note`'s tag with a `new_tag`
